...
See "High-risk feature categories" below for other categories (encryption, telemetry, accounts, running third-party code) that also never get queued or implemented without the maintainer's own explicit, specific sign-off — this scan should recognize and skip those the same way, not just sync.
...
- **Telemetry, analytics, or any network call this app doesn't already make.** Already a hard no for telemetry specifically per `PHILOSOPHY.md`; any other new outbound network call this app doesn't already have a documented reason for needs the same treatment.
...
None of this means "never build these" forever — encryption in particular is a reasonable thing a future version of this app could want. It means: if the competitor scan sees one of these, skip queueing it (for sync) or queue it but flag prominently in the `ROADMAP.md` entry that it needs the maintainer's explicit sign-off before implementation, not the routine's own judgment (for the others); if the feature-implementation routine's work order ever contains one of these categories, it stops and records that it's blocked on maintainer sign-off rather than implementing its own best guess at the design.
