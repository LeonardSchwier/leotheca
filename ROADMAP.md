...
- 🚧 **Search Content-Read Crash** (claim: Claude-Code-interactive-20260828T0945Z, 2026-08-28T09:45Z, direct-to-main, live maintainer-supervised session, no separate branch/PR): confirmed on-device on a real ~500-note vault: a search whose query needs content from many non-name-matching files still triggers an Android `OutOfMemoryError` after ~1700+ sequential one-file-at-a-time native content-read calls. Search's own directory walk was already fixed the same day (`find_all_files`, one native call instead of one per directory); fixing this remaining piece by batching multiple files' content into fewer native calls (`read_text_files_batch`) too.
