...
### Local automation: `leotheca://` URLs (desktop only)

`src/app/automationCommands.ts` (pure `parseAutomationUrl`) and the effect in `App.tsx` that wires it up implement `ROADMAP.md`'s "Local Automation Commands": `leotheca://read-current-note` copies the active note's content to the clipboard, `leotheca://new-note?content=...` creates a note from the given (URL-decoded) text. No network involved, offline-by-design is unaffected: the OS's own URL-scheme dispatch is the entire transport, the same way a `mailto:` link works. Desktop (Tauri) only; there's no Android/Capacitor equivalent, and the effect that registers the listeners is skipped there via the same `Capacitor.isNativePlatform()` check `tauriBridge.ts` uses.
