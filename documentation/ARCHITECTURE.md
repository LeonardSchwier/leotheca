...
- `android/app/src/main/java/.../LeothecaWidgetProvider.java`: declares the two offline home-screen actions. It launches the app with the same local URL commands that the app already parses, so the widget never needs direct workspace access or a second command implementation.
