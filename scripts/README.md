...
This directory contains utility scripts to help coding agents follow the autonomous multi-agent coordination protocol defined in `skills/multi-agent-autonomous-coordination.md`.
...
./scripts/check-abandoned-claims.sh

# Custom threshold
./scripts/check-abandoned-claims.sh --hours 6

# With reclaim mode (shows reclaim procedure)
./scripts/check-abandoned-claims.sh --reclaim MistralVibe-cloud-ABC123
```

**What it does**:
- Scans ROADMAP.md for all `🚧` (claimed) items
...
curl -s -H "Authorization: token YOUR_GITHUB_TOKEN" \
  "https://api.github.com/repos/LeonardSchwier/leotheca/actions/runs?branch=agent/your-work-item&per_page=1"
```
...
```bash
# 1. Startup
./scripts/check-abandoned-claims.sh --reclaim YOUR-AGENT-ID
...
./scripts/check-abandoned-claims.sh --hours 2  # Use shorter threshold
```
...
4. **Update scripts when protocols change**:
   ...
   - Keep scripts in sync with `CONSTITUTION.md` and skill files
...
### Pattern 3: Verifying Main Sync

```bash
# Are you in sync with main?
git fetch origin main
...
- Use `./scripts/script-name.sh` not `script-name.sh`

### Script doesn't find abandoned claims
- Check that ROADMAP.md has the correct format
- Ensure you've fetched latest remote state
- Verify branches exist on remote

### Script reports false positives
- Human work might be detected as abandoned
- Always manually verify before reclaiming
- Check commit authors and timestamps carefully

## Contributing Improvements

If you find a bug in a script or can improve it:
1. Fix the script
2. Test the fix
3. Commit the improvement (no ROADMAP.md claim needed for script improvements)
4. Update this documentation if the interface changed