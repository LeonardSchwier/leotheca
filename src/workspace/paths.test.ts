...
describe("resolvePath", () => {
  it("resolves a plain relative target against the base directory", () => {
    expect(resolvePath("/vault/notes", "image.png")).toBe(
      "/vault/notes/image.png",
    );
  });
  ...
  it("treats a leading ./ as a no-op", () => {
    expect(resolvePath("/vault/notes", "./sub/image.png")).toBe(
      "/vault/notes/sub/image.png",
    );
  });

  it("returns an already-absolute target untouched", () => {
    expect(resolvePath("/vault/notes", "/elsewhere/image.png")).toBe(
      "/elsewhere/image.png",
    );
  });
...
describe("resolvePathWithinWorkspace", () => {
  it("keeps a nested relative target inside the workspace", () => {
    expect(
      resolvePathWithinWorkspace("/vault", "/vault/notes", "images/cat.png"),
    ).toBe("/vault/notes/images/cat.png");
  });
  ...
  it("allows a sibling attachment that remains inside the workspace", () => {
    ...
    ).toBe("/vault/attachments/cat.png");
  });

  it("rejects traversal that escapes the workspace root", () => {
    expect(
      resolvePathWithinWorkspace("/vault", "/vault/notes", "../../outside.png"),
    ).toBeNull();
  });
  ...
  it("rejects an out-of-workspace base even if its target would resolve back inside", () => {
    expect(
      resolvePathWithinWorkspace("/vault", "/outside", "../vault/cat.png"),
    ).toBeNull();
  });

  it("rejects absolute targets before resolution", () => {
    expect(
      resolvePathWithinWorkspace("/vault", "/vault/notes", "/outside/cat.png"),
    ).toBeNull();
    expect(
      resolvePathWithinWorkspace(
        "C:/vault",
  ...
  it("preserves a valid Windows-style workspace path", () => {
    ...
    ).toBe("C:/vault/attachments/cat.png");
  });
});

describe("relativePathBetween", () => {
  it("returns a bare file name for a file in the same directory", () => {
    expect(relativePathBetween("/vault/notes", "/vault/notes/image.png")).toBe(
      "image.png",
    );
  });
  ...
  it("climbs up and back down for a sibling directory", () => {
    expect(
      relativePathBetween("/vault/notes/sub", "/vault/attachments/image.png"),
...
describe("isPathWithinWorkspace", () => {
  it("accepts the workspace root itself", () => {
    expect(isPathWithinWorkspace("/vault", "/vault")).toBe(true);
  });

  it("accepts a nested path inside the workspace", () => {
    expect(isPathWithinWorkspace("/vault", "/vault/notes/today.md")).toBe(true);
  });

  it("rejects a path entirely outside the workspace", () => {
    expect(isPathWithinWorkspace("/vault", "/etc/passwd")).toBe(false);
  });

  it("rejects a sibling directory that merely shares a name prefix", () => {
    expect(isPathWithinWorkspace("/vault", "/vault-evil/secret.md")).toBe(
      false,
    );
  });

  it("rejects a relative (non-absolute) path", () => {
    expect(isPathWithinWorkspace("/vault", "notes/today.md")).toBe(false);
  });
});
