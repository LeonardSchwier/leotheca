 ...
 * - Android: not supported here; Android dictation goes through
 *   `startAndroidSpeechRecognition`'s native `SpeechRecognizer` result
 *   instead (see `speechController.ts`'s `stop()`), which never calls
 *   this function.
 */
